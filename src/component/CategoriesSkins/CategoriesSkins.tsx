      ...
      <ul className='flex flex-col gap-7 '>
        {data?.categories?.map(el => (
          <li key={el.name} className=' flex flex-col'>
            ...
            {el?.items?.map(e => (
              <div key={e.name} className="bg-[#23272a] px-3 cursor-pointer py-6 items-center rounded-md w-[230px]">
                <img width={120} height={120} src={e.imageUrl} alt="" />
                <h1 className=" text-[14px] font-semibold text-[#747f87] text-left">{el.name}</h1>
              </div>
            ))}
            </div>
          </li>
